...
//紅色字體為判斷密碼是否填寫正確
if($_SESSION['username'] != null && $opw2 != null && $opw == $opw2){
        $id = $_SESSION['username'];
    
        //更新資料庫資料語法
        $sql = "UPDATE `project2017c`.$sidt SET `phone` = '$pho',`mail` = '$mail',`address` = '$address' WHERE $sidt.`username` = '$id'";
        if(mysql_query($sql)){
                echo '修改成功!<br>';
...
else if($opw == null || $opw2 == null){
	echo '請確認密碼是否正確填寫<br>';
	echo '<meta http-equiv=REFRESH CONTENT=10;url=infoupdate.php>';
	echo '<a href="infoupdate.php">系統將自動轉跳，或按此手動轉跳。</a>';
}
else if($opw2 != $opw){
	echo '請確認原密碼是否正確<br>';
	echo '<meta http-equiv=REFRESH CONTENT=10;url=infoupdate.php>';
